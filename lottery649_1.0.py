...
# 如要將查詢資訊寫入資料庫需自行新增mysql資料庫並建立 Lotto649的資料表


# 大樂透
def Lotto649(savedata, updatedb):
    url = 'http://www.taiwanlottery.com.tw/Lotto/Lotto649/history.aspx'
    # 查詢y年m月的開獎號碼
    for y in range(start_year, int(last_year) + 1):
        for m in range(1, 13):
            ...
            if not nodata_flog:
                ...
                # 確認是否有資料
                nodatapat = re.compile('<span id="Lotto649Control_history_Label1".*?>(.*?)</span>')
                ...
                DrawTermpat = re.compile(
                    r'<span id="Lotto649Control_history_dlQuery_L649_DrawTerm_\d{1,2}">(.*?)</span>')
                DrawTerm = DrawTermpat.findall(lottodata.text)
                # 開獎日
                DDatepat = re.compile(r'<span id="Lotto649Control_history_dlQuery_L649_DDate_\d{1,2}">(.*?)</span>')
                DDate = DDatepat.findall(lottodata.text)
                # 兌獎截止日
                EDatepat = re.compile(r'<span id="Lotto649Control_history_dlQuery_L649_EDate_\d{1,2}">(.*?)</span>')
                EDate = EDatepat.findall(lottodata.text)
                # 開獎大小順序 前6個號碼為第一區數字 第7個號碼為第二區數字
                No1pat = re.compile(r'<span id="Lotto649Control_history_dlQuery_SNo1_\d{1,2}">(.*?)</span>')
                No1 = No1pat.findall(lottodata.text)
                No2pat = re.compile(r'<span id="Lotto649Control_history_dlQuery_SNo2_\d{1,2}">(.*?)</span>')
                No2 = No2pat.findall(lottodata.text)
                No3pat = re.compile(r'<span id="Lotto649Control_history_dlQuery_SNo3_\d{1,2}">(.*?)</span>')
                No3 = No3pat.findall(lottodata.text)
                No4pat = re.compile(r'<span id="Lotto649Control_history_dlQuery_SNo4_\d{1,2}">(.*?)</span>')
                No4 = No4pat.findall(lottodata.text)
                No5pat = re.compile(r'<span id="Lotto649Control_history_dlQuery_SNo5_\d{1,2}">(.*?)</span>')
                No5 = No5pat.findall(lottodata.text)
                No6pat = re.compile(r'<span id="Lotto649Control_history_dlQuery_SNo6_\d{1,2}">(.*?)</span>')
                No6 = No6pat.findall(lottodata.text)
                No7pat = re.compile(r'<span id="Lotto649Control_history_dlQuery_SNo_\d{1,2}">(.*?)</span>')
                ...
                for i in range(len(DrawTerm) - 1, -1, -1):
                    ...
                    if updatedb:
                        try:
                            sql = "insert into lotto_lotto649(drawterm,no1,no2,no3,no4,no5,no6,no7,ddate,edate) values (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"
                    ...
                    print("第%s期大樂透, 開獎日期:%s,兌獎截止日期:%s" % (DrawTerm[i], DDate[i], EDate[i]))
                    print("號碼大小順序為 :%s,%s,%s,%s,%s,%s,特別號號碼為:%s" % (
...
def main():
    ...
    # 抓取大樂透資料
    Lotto649(savedata=False,updatedb = False)
